...
function inject (bot, { version, storageBuilder, hideErrors }) {
  ...
  bot._client.on('explosion', (packet) => {
    ...
    if (packet.affectedBlockOffsets) {
      // 1.21.3+ omits this field; block destruction arrives via normal block_change packets instead.
  ...
  async function switchWorld () {
    if (bot.world) {
      ...
      for (const [x, z] of Object.keys(bot.world.async.columns).map(key => key.split(',').map(x => Number.parseInt(x, 10)))) {
